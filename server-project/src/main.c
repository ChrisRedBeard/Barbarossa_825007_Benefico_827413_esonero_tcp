...
int main(int argc, char *argv[]) {
	...
	int port = SERVER_PROTOPORT; // valore di default
		...
		if (strcmp(argv[i], "-p") == 0) {
			if (i + 1 < argc) {
				...
				if (port <= 0) {
					...
					printf("Uso corretto:\n");
					printf("  main.exe [-p porta]\n\n");
					return 1;
				}
				i++; // Salta il numero della porta
			} else {
	...
	server_addr.sin_port = htons(port); // converte un numero del formato del computer a quello della rete (Big-Endian)
	server_addr.sin_addr.s_addr = htonl(INADDR_ANY);

	// TODO: Bind socket
	if ( // la funzione bind associa alla socket un indirizzo in modo da poter essere contattata da un client
	bind(my_socket, (struct sockaddr*) &server_addr, sizeof(server_addr)) < 0) {
